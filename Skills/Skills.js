...
function Awake () {
	...
	playerBob = gameController.targetObjects[35];
	...
	playerTarget = new GameObject("PlayerSkillTarget");
	skillTargets = new Transform[3];
	skillTargets[1] = playerTarget.transform;
	seeking = false;
	...
	if(playerData.characterSelected == 0) {
		for(var runtCount : int; runtCount < 6; runtCount++) {
			...
			runts[runtCount].name = "Runt" + runtCount;
			runts[runtCount].animation["Walk_Loop"].speed = 2;
			runtPaths[runtCount] = runts[runtCount].GetComponent(AIPath);
			runtTargetObjects[runtCount] = new GameObject("RuntTarget" + runtCount);
...
function Start () {
	...
	if(gameController.characterSelected[1] == 2){
		skillParticles[1,0] = GameObject.FindGameObjectWithTag("Player").transform.Find("Particles").Find("Flame").gameObject;
		skillParticles[1,1] = GameObject.FindGameObjectWithTag("Player").transform.Find("Particles").Find("FreezingFog").gameObject;
		skillParticles[1,1].GetComponent(FreezingFog).BreakOff();
	}
	else if(gameController.characterSelected[1] == 3){
		skillParticles[1,0] = GameObject.FindGameObjectWithTag("Player").transform.Find("Particles").Find("BlackHole").gameObject;
		skillParticles[1,0].GetComponent(BlackHole).BreakOff();
	}
	
	//if leonardo
	if(gameController.characterSelected[1] == 2)	{
		StartCoroutine("BuffAura", 1);
	}
...
function SelfPathFinish (charNumber : int) {
	//ensure correct pathing	
	gameController.currentTargets[charNumber] = gameController.originalTarget;
	...
	for(var i : int = 0; i < 61; i++)	{
		if(i == 60)		{
			...
			if(unitNumber == 1 || unitNumber == 2)	{
				opponentSkills.InterruptSkills(unitNumber, gameController.currentTargets[unitNumber]);
			}
	...
	for(var i : int = 0; i < (60 * gameController.skillLevels[charNumber,2]); i++)	{
		...
		if((gameController.currentResources[charNumber] < gameController.skillCosts[charNumber,2] && i > 59) || gameController.currentHealths[targetNumber] < 1
			...
			if(targetNumber == 1 || targetNumber == 2)		{
				opponentSkills.InterruptSkills(targetNumber, gameController.currentTargets[targetNumber]);
			}
...
function InterruptSkills (unitNumber : int, target : int) {
	...
	gameController.playerAiming = false;
	seeking = false;
